...
internal sealed class RestApiOperationRunner
{
	...
	private const string MEDIA_TYPE_TEXT_PLAIN = "text/plain";

	private const string DEFAULT_RESPONSE_KEY = "default";

	/// <summary>
	/// HTTP request method.
	/// </summary>
	private const string HTTP_REQUEST_METHOD = "http.request.method";

	/// <summary>
	/// The HTTP request payload body.
	/// </summary>
	private const string HTTP_REQUEST_BODY = "http.request.body";

	/// <summary>
	/// The HTTP request options.
	/// </summary>
	private const string HTTP_REQUEST_OPTIONS = "http.request.options";

	/// <summary>
	/// Absolute URL describing a network resource according to RFC3986.
	/// </summary>
	private const string URL_FULL = "url.full";
		...
		RestApiOperationServerUrlValidationOptions? serverUrlValidationOptions = null)
	{
		...
		_userAgent = userAgent ?? OpenApiFunctionExecutionParameters.HTTPHEADERCONSTANT_USERAGENT;
		...
		{
			{ MEDIA_TYPE_APPLICATION_JSON, BuildJsonPayload },
			{ MEDIA_TYPE_TEXT_PLAIN, BuildPlainTextPayload }
		};
	}
	...
	{
		var url = _urlFactory?.Invoke(operation, arguments, options) ?? this.BuildsOperationUrl(operation, arguments, options?.ServerUrlOverride, options?.ApiHostUrl);

		this.ValidateUrl(url);

		var headers = _headersFactory?.Invoke(operation, arguments, options) ?? operation.BuildHeaders(arguments);

		var (payload, content) = _payloadFactory?.Invoke(operation, arguments, _enableDynamicPayload, _enablePayloadNamespacing, options) ?? this.BuildOperationPayload(operation, arguments);
		...
		CancellationToken cancellationToken = default)
	{
		...
		requestMessage.Options.Set(OpenApiOperationFunctionContext.KernelFunctionContextKey, new OpenApiOperationFunctionContext(options?.Arguments));
		...
		requestMessage.Headers.Add("User-Agent", !string.IsNullOrWhiteSpace(_userAgent)
			? _userAgent
			: OpenApiFunctionExecutionParameters.HTTPHEADERCONSTANT_USERAGENT);
		...
		catch (HttpRequestException ex)
		{
			...
			exception.Data.Add(HTTP_REQUEST_METHOD, requestMessage.Method.Method);
			exception.Data.Add(URL_FULL, requestMessage.RequestUri?.ToString());
			exception.Data.Add(HTTP_REQUEST_BODY, payload);
		...
		{
			ex.Data.Add(HTTP_REQUEST_METHOD, requestMessage.Method.Method);
			ex.Data.Add(URL_FULL, requestMessage.RequestUri?.ToString());
			ex.Data.Add(HTTP_REQUEST_BODY, payload);
	...
	private async Task<RestApiOperationResponse> ReadContentAndCreateOperationResponseAsync(HttpRequestMessage requestMessage, HttpResponseMessage responseMessage, object? payload, CancellationToken cancellationToken)
	{
		...
		var mediaType = contentType?.MediaType ?? throw new OpenApiException("No media type available.");
	...
	private (object? Payload, HttpContent? Content) BuildOperationPayload(RestApiOperation operation, IDictionary<string, object?> arguments)
	{
		...
		{
			if (!arguments.TryGetValue(RestApiOperation.ContentTypeArgumentName, out var fallback) || fallback is not string mediaTypeFallback)
			{
				throw new OpenApiException($"No media type is provided for the {operation.Id ?? operation.ToString()} operation.");
			}
		...
		{
			throw new OpenApiException($"The media type {mediaType} of the {operation.Id ?? operation.ToString()} operation is not supported by {nameof(RestApiOperationRunner)}.");
		}
	...
	private (object Payload, HttpContent Content) BuildJsonPayload(RestApiPayload? payloadMetadata, IDictionary<string, object?> arguments)
	{
		if (_enableDynamicPayload)
		{
			if (payloadMetadata is null)
				throw new OpenApiException("Payload can't be built dynamically due to the missing payload metadata.");

			var payload = this.BuildJsonObject(payloadMetadata.Properties, arguments);

			return (payload, new StringContent(payload.ToJsonString(), Encoding.UTF8, MEDIA_TYPE_APPLICATION_JSON));
		}

		// Get operation payload content from the 'payload' argument if dynamic payload building is not required.
		if (!arguments.TryGetValue(RestApiOperation.PayloadArgumentName, out var argument) || argument is not string content)
		{
			throw new OpenApiException($"No payload is provided by the argument '{RestApiOperation.PayloadArgumentName}'.");
		}

		return (content, new StringContent(content, Encoding.UTF8, MEDIA_TYPE_APPLICATION_JSON));
	}
	...
	private JsonObject BuildJsonObject(IList<RestApiPayloadProperty> properties, IDictionary<string, object?> arguments, string? propertyNamespace = null)
	{
		...
		foreach (var propertyMetadata in properties)
		{
			...
			if (propertyMetadata.Type == Microsoft.OpenApi.JsonSchemaType.Object)
			{
			...
			if (!string.IsNullOrEmpty(propertyMetadata.ArgumentName) && arguments.TryGetValue(propertyMetadata.ArgumentName!, out var argument) && argument is not null)
			{
			...
			{
				throw new OpenApiException($"No argument is found for the '{propertyMetadata.Name}' payload property.");
			}
		}
	...
	private static KernelJsonSchema? GetExpectedSchema(IDictionary<string, KernelJsonSchema?>? expectedSchemas, HttpStatusCode statusCode)
	{
		...
		if (expectedSchemas is not null)
		{
			...
			// Default
			matchingResponse ??= expectedSchemas.FirstOrDefault(r => r.Key == DEFAULT_RESPONSE_KEY).Value;
		}
	...
	{
		if (!arguments.TryGetValue(RestApiOperation.PayloadArgumentName, out var argument) || argument is not string payload)
		{
			throw new OpenApiException($"No argument is found for the '{RestApiOperation.PayloadArgumentName}' payload content.");
		}

		return (payload, new StringContent(payload, Encoding.UTF8, MEDIA_TYPE_TEXT_PLAIN));
	}
	...
	private async Task<object?> ReadHttpContentAsync(HttpRequestMessage requestMessage, HttpResponseMessage responseMessage, string mediaType, CancellationToken cancellationToken)
	{
		...
		if (content is null)
		{
			...
			if (!s_contentReaderByContentType.TryGetValue(mediaType, out var reader))
			{
				...
				{
					throw new OpenApiException($"The string `{mediaType}` is not a valid media type.");
				}
				...
				{
					throw new OpenApiException($"The content type `{mediaType}` is not supported.");
				}
			}
	...
	{
		IDictionary<string, object?>? requestOptions = requestMessage.Options;

		if (requestOptions is not null)
		{
			exception.Data[HTTP_REQUEST_OPTIONS] = requestOptions;
		}
	}
