...
internal static partial class RestApiOperationExtensions
{
	...
	public static RestApiExpectedResponse? GetDefaultReturnParameter(this RestApiOperation operation, string[]? preferredResponses = null)
	{
		return GetDefaultResponse(operation.Responses, preferredResponses ??= s_preferredResponses);
	}
	...
	private static List<RestApiParameter> GetPayloadParameters(RestApiOperation operation, bool useParametersFromMetadata, bool enableNamespacing, RestApiParameterFilter? parameterFilter)
	{
		if (useParametersFromMetadata)
		{
			...
			{
				throw new OpenApiException($"Payload parameters cannot be retrieved from the '{operation}' operation payload metadata because it is missing.");
			}
			...
			if (operation.Payload.MediaType == MEDIA_TYPE_TEXT_PLAIN)
			{
	...
	private static RestApiParameter CreatePayloadArtificialParameter(RestApiOperation operation)
	{
		return new RestApiParameter(
			...
			operation.Payload?.MediaType == MEDIA_TYPE_TEXT_PLAIN ? JsonSchemaType.String : JsonSchemaType.Object,
			isRequired: true,
			expand: false,
			RestApiParameterLocation.Body,
			RestApiParameterStyle.Simple,
	...
	private static List<RestApiParameter> GetParametersFromPayloadMetadata(RestApiOperation operation, object parent, IList<RestApiPayloadProperty> properties, bool enableNamespacing = false, RestApiParameterFilter? parameterFilter = null, string? rootPropertyName = null)
	{
		...
		foreach (var property in properties)
		{
			...
			if (!property.Properties.Any())
			{
				...
				var parameter = new RestApiParameter(
					...
					schema: GetSchemaWithType(property.Schema, property.Type))
				{
		...
		if (properties.Count == 0 && parent is RestApiPayload)
			parameters.Add(CreatePayloadArtificialParameter(operation));
	...
	private static KernelJsonSchema? GetSchemaWithType(KernelJsonSchema? schema, JsonSchemaType type)
	{
		if (schema is not null && !schema.RootElement.TryGetProperty(TYPE_PROPERTY_NAME, out var _))
		{
			var originalSchema = JsonSerializer.Serialize(schema.RootElement);
			if (JsonNode.Parse(originalSchema) is JsonObject obj)
			{
				obj.Add(TYPE_PROPERTY_NAME, GetSchemaTypeAsString(type));
				schema = KernelJsonSchema.Parse(obj.ToString());
			}
		}
	...
	private static string GetSchemaTypeAsString(JsonSchemaType type)
	{
		if (BitOperations.PopCount((uint)type) == 1)
			return type.ToString();

		var result = type.ToString();

		foreach (var flag in Enum.GetValues<JsonSchemaType>())
		{
